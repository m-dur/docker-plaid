...
def process_accounts(accounts, bank_balances, credit_cards, additional_fields=None):
    ...
    for account in accounts:
        # Get matching balance data
        balance_data = next((b for b in bank_balances if b.account_id == account.account_id), None)
        ...
        account_data = {
            ...
            'institution_id': additional_fields.get('institution_id'),
            'type': str(account.type),
            'subtype': str(account.subtype),
            'mask': account.mask,
            'verification_status': getattr(account, 'verification_status', None),
            'currency': account.balances.iso_currency_code,
            'balance_current': float(account.balances.current) if account.balances.current is not None else None,
            'balance_available': float(account.balances.available) if account.balances.available is not None else None,
            'balance_limit': float(account.balances.limit) if account.balances.limit is not None else None,
            ...
            'apr_type': str(credit_data.aprs[0].apr_type) if credit_data and credit_data.aprs else None,
